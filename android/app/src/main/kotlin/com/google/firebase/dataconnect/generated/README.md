...
with service ID `dahabhubgoogle-copy3`
hosted in `us-east4`.
...
is the query named "GetUserBookings",
which can be accessed via the [ExampleConnector.getUserBookings] property.


An example of the property for a mutation
is the mutation named "CreateBooking",
which can be accessed via the [ExampleConnector.createBooking] property.
...
For example, the "GetUserBookings" query has no variables
and can be executed via the
[ExampleConnector.getUserBookings]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val queryResult = connector.getUserBookings.execute()
println("GetUserBookings query returned: ${queryResult.data}")
```
...
For example, the "CreateBooking" mutation has 3 required variables ("eventId", "ticketTypeId", and "quantity")
and can be executed via the [ExampleConnector.createBooking]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val mutationResult = connector.createBooking.execute(eventId=java.util.UUID.randomUUID(), ticketTypeId=java.util.UUID.randomUUID(), quantity=5523)
println("CreateBooking mutation returned: ${mutationResult.data}")
```
